...
<div class="container" style="background-color: white">
    ...
    <div class="mb-3 row">
        <legend class="col-sm-6"><?= __('Edit School') ?></legend>
    ...
    <div class="mb-3 row">
        <label for="last_name" class="col-sm-2 col-form-label">Province</label>
        <div class="col-sm-6">
            <select name="province_id"  class="form-control">
                <option>-Please Select</option>
                <?php foreach ($provinces as $province): ?>
                    <option <?= ($province->id == $school->province_id ? 'selected' : '') ?> value="<?= $province->id ?>"><?= $province->province_name ?></option>
                <?php endforeach; ?>
            </select>
        </div>
    </div>
